				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Wave Hub Premium ")
							if not fold then
								makefolder("./Wave Hub Premium ")
							end
							fold = isfolder("./Wave Hub Premium Scripts/Themes")
							if not fold then
								makefolder("./Wave Hub Premium Scripts/Themes")
							end
							fold = isfolder("./Wave Hub Premium Scripts/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./Wave Hub Premium Scripts Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Wave Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Wave Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Wave Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
